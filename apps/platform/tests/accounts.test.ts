...
test('email verification is hashed, expiring and single-use',()=>{
 const base=loadConfig();const databasePath=path.join(base.platformRoot,'data',`verify-email-test-${process.pid}.sqlite`);const config=loadConfig({databasePath});remove(databasePath);const database=openDatabase(config);
 try{migrateDatabase(database);seedDatabase(database,config,'Existing-Demo-Password!');database.prepare('UPDATE users SET email_verified_at=NULL WHERE id=?').run('user-employee-demo-0001');const service=new AccountLifecycleService(database,config);const now=new Date('2026-07-18T17:10:00.000Z');const verification=service.createEmailVerification('user-employee-demo-0001',now);assert.ok(verification.token);const stored=String(database.prepare("SELECT token_hash FROM account_tokens WHERE user_id=? AND token_type='VERIFY_EMAIL'").get('user-employee-demo-0001')?.token_hash);assert.notEqual(stored,verification.token);assert.equal(service.verifyEmail(verification.token??'',new Date(now.getTime()+60_000)),'user-employee-demo-0001');assert.ok(database.prepare('SELECT email_verified_at FROM users WHERE id=?').get('user-employee-demo-0001')?.email_verified_at);assert.throws(()=>service.verifyEmail(verification.token??'',new Date(now.getTime()+120_000)),/INVALID_OR_EXPIRED_TOKEN/);assert.deepEqual(service.createEmailVerification('user-employee-demo-0001'),{token:null,email:'funcionario1@demo.invalid'});}
 finally{database.close();remove(databasePath);}
});
