...
test('demo TOTP setup encrypts the secret and enables MFA after confirmation',()=>{
 ...
 try{migrateDatabase(database);seedDatabase(database,config,'Existing-Demo-Password!');const service=new AccountLifecycleService(database,config);const now=new Date('2026-07-18T17:30:00.000Z');const setup=service.startMfa('user-demo-admin',now);assert.match(setup.provisioningUri,/^otpauth:\/\/totp\//);assert.equal(setup.recoveryCodes.length,8);const stored=String(database.prepare('SELECT secret_ciphertext FROM mfa_credentials WHERE user_id=?').get('user-demo-admin')?.secret_ciphertext);assert.equal(stored.includes(setup.secret),false);service.confirmMfa('user-demo-admin',service.currentTotpForDemo('user-demo-admin',now),now);assert.equal(Number(database.prepare('SELECT enabled FROM mfa_credentials WHERE user_id=?').get('user-demo-admin')?.enabled),1);}
 finally{database.close();remove(databasePath);}
});
