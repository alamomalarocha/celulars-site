...
export class DeliveryOutboxService {
  ...
  queue(actor:Principal,input:{channel:'EMAIL'|'WHATSAPP';templateCode:string;recipient:string;variables:Readonly<Record<string,string>>;companyId?:string|null;conversationId?:string|null;correlationId?:string|null},now=new Date()):object{
    ...
    this.database.prepare(`INSERT INTO delivery_outbox (id,channel,template_code,recipient,subject,body,company_id,conversation_id,correlation_id,status,next_attempt_at,created_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,'PENDING',?,?,?,?)`).run(id,input.channel,input.templateCode,recipient,subject,body,input.companyId??actor.companyId??null,input.conversationId??null,input.correlationId??null,timestamp,actor.userId,timestamp,timestamp);
