...
const queueCompanyEmail = (companyId:string,templateCode:string,variables:Record<string,string>,correlationId:string):object|null => {
  ...
  async function handler(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    try {
      ...
      if (method === 'GET' && url.pathname === '/api/documents') { sendJson(response,200,documents.list(principal));return; }
      ...
      if(method==='GET'&&url.pathname==='/api/admin/users/options'){sendJson(response,200,userAdmin.options(principal));return;}
      ...
      const adminUserSessions=url.pathname.match(/^\/api\/admin\/users\/([^/]+)\/sessions$/);
      if(method==='GET'&&adminUserSessions?.[1]){sendJson(response,200,userAdmin.sessions(principal,decodeURIComponent(adminUserSessions[1])));return;}
      if(method==='DELETE'&&adminUserSessions?.[1]){const userId=decodeURIComponent(adminUserSessions[1]);const result=userAdmin.revokeSessions(principal,userId);recordAudit(database,principal,'SESSIONS_REVOKED','USER',userId,auditContext(request,result));sendJson(response,200,result);return;}      const adminUserHistory=url.pathname.match(/^\/api\/admin\/users\/([^/]+)\/history$/);
