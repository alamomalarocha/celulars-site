...
import { qrSvgDataUrl } from './qr-svg.js';
...
export class AccountLifecycleService {
  ...
  createEmailVerification(userId:string,now=new Date()):{token:string|null;email:string|null}{
    const user=this.database.prepare("SELECT email,email_verified_at FROM users WHERE id=? AND status='ACTIVE'").get(userId) as {email:string;email_verified_at:string|null}|undefined;
    if(!user||user.email_verified_at)return{token:null,email:user?.email??null};
    this.database.prepare("UPDATE account_tokens SET used_at=? WHERE user_id=? AND token_type='VERIFY_EMAIL' AND used_at IS NULL").run(now.toISOString(),userId);
    const token=randomToken();
    this.database.prepare("INSERT INTO account_tokens (id,user_id,token_type,token_hash,expires_at,created_at) VALUES (?,?,'VERIFY_EMAIL',?,?,?)").run(randomUUID(),userId,digest(this.config,token),new Date(now.getTime()+TOKEN_TTL_MS).toISOString(),now.toISOString());
    return{token,email:user.email};
  }
  verifyEmail(token:string,now=new Date()):string{
    const row=this.database.prepare("SELECT id,user_id FROM account_tokens WHERE token_type='VERIFY_EMAIL' AND token_hash=? AND used_at IS NULL AND expires_at>?").get(digest(this.config,token),now.toISOString()) as {id:string;user_id:string}|undefined;
    if(!row)throw new Error('INVALID_OR_EXPIRED_TOKEN');
    this.database.exec('BEGIN IMMEDIATE');
    try{this.database.prepare('UPDATE users SET email_verified_at=?,updated_at=? WHERE id=?').run(now.toISOString(),now.toISOString(),row.user_id);this.database.prepare('UPDATE account_tokens SET used_at=? WHERE id=?').run(now.toISOString(),row.id);this.database.exec('COMMIT');return row.user_id;}catch(error){this.database.exec('ROLLBACK');throw error;}
