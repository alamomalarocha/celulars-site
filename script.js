(function clientMain(){
  ...
  const MAIN_TABLE_MODELS = new Set(['iPhone 12','iPhone 12 mini','iPhone 12 Pro','iPhone 12 Pro Max','iPhone 13','iPhone 13 mini','iPhone 13 Pro','iPhone 13 Pro Max','iPhone 14','iPhone 14 Plus','iPhone 14 Pro','iPhone 14 Pro Max','iPhone 15','iPhone 15 Plus','iPhone 15 Pro','iPhone 15 Pro Max','iPhone 16e','iPhone 16','iPhone 16 Plus','iPhone 16 Pro','iPhone 16 Pro Max','iPhone 17e','iPhone 17','iPhone Air','iPhone 17 Pro','iPhone 17 Pro Max']);
  ...
  function messageFor(m,s){return s.capacity&&s.color&&s.condition?'Olá, tenho interesse no '+m.model_name+', capacidade '+s.capacity+', cor '+s.color+', condição '+s.condition+', pela CELULARS. Gostaria de confirmar cores disponíveis, disponibilidade, preço final e condições de compra.':s.capacity?'Olá, tenho interesse no '+m.model_name+', capacidade '+s.capacity+', pela CELULARS. Gostaria de confirmar cores disponíveis, condição, preço final e condições de compra.':'Olá, tenho interesse no '+m.model_name+' pela CELULARS. Gostaria de confirmar capacidades, cores, disponibilidade, preço final e condições de compra.'}
  ...
  function matchesRowSearch(row){if(!activeSearch)return true;const hay=[row.model.model_name,row.model.year,row.model.line,row.capacity,row.model.colors.join(' '),row.model.conditions.join(' ')].join(' ');return normalize(hay).includes(normalize(activeSearch))}
  function tableRowsFor(models){return models.filter(matchesModelFilters).flatMap(m=>m.capacities.map(capacity=>({model:m,capacity}))).filter(matchesRowSearch)}
  ...
  function renderConditionPrice(m,capacity,condition,selected){const p=capacityConditionPrice(m,capacity,condition),label=normalize(condition).includes('ecpo')?'eCPO':'Novo';return'<button class="price-option'+(selected?' is-selected':'')+'" type="button" data-option="condition" data-value="'+escapeHtml(condition)+'" aria-pressed="'+(selected?'true':'false')+'"><span>'+label+'</span>'+(p?'<strong>'+formatUsd(p)+'</strong><em>'+formatBrl(p*CATALOG_RATE)+'</em>':'<strong>Sob consulta</strong><em>confirmar valor final</em>')+'</button>'}
  function renderPriceCells(m,s){return{ecpo:renderConditionPrice(m,s.capacity,'eCPO eco Certified Pre-Owned',normalize(s.condition).includes('ecpo')),novo:renderConditionPrice(m,s.capacity,'Novo',normalize(s.condition)==='novo')}}
  function selectedPriceText(m,s){const p=capacityConditionPrice(m,s.capacity,s.condition),general=basePrice(m);if(p)return formatUsd(p)+' / '+formatBrl(p*CATALOG_RATE);return general?'Sob consulta; referência geral a partir de '+formatUsd(general)+' / '+formatBrl(general*CATALOG_RATE):'Sob consulta'}
  ...
  function renderCapacityCell(capacity){return'<span class="capacity-pill">'+escapeHtml(capacity)+'</span>'}
  ...
  function renderCatalogRow(row){const m=row.model,s={capacity:row.capacity,color:m.colors[0],condition:activeCondition==='todos'?'Novo':activeCondition},cells=renderPriceCells(m,s),i=IPHONE_MODELS.indexOf(m);return'<tr '+rowDataAttrs(m,i,s)+'><td data-label="Modelo">'+renderModelCell(m,s)+'</td><td data-label="Ano"><span class="year-badge">'+m.year+'</span></td><td data-label="Capacidade">'+renderCapacityCell(s.capacity)+'</td><td data-label="Cores disponíveis">'+compactOptionList('color',m.colors,s.color)+'</td><td data-label="Preço eCPO" data-price-cell="ecpo">'+cells.ecpo+'</td><td data-label="Preço Novo" data-price-cell="novo">'+cells.novo+'</td><td data-label="Status"><span class="status-pill '+statusClass(m)+'">Status: '+escapeHtml(m.stock_status||'Consultar disponibilidade')+'</span></td><td data-label="Ação">'+renderActions(m,s)+'</td></tr>'}
  function renderTable(rows){return'<div class="catalog-table-wrap"><table class="catalog-table"><thead><tr><th>Modelo</th><th>Ano</th><th>Capacidade</th><th>Cores disponíveis</th><th>Preço eCPO</th><th>Preço Novo</th><th>Status</th><th>Ação</th></tr></thead><tbody>'+rows.map(renderCatalogRow).join('')+'</tbody></table></div>'}
  function renderLegacyConsult(){return'<section class="legacy-consult-section legacy-consult-note"><div class="section-heading"><span>Consulta especial</span><strong>Modelos anteriores sob consulta</strong></div><p>Modelos anteriores ao iPhone 12 podem ser consultados diretamente pelo WhatsApp, conforme disponibilidade.</p><a class="whatsapp-button" href="'+buildWhatsAppUrl('Olá, tenho interesse em consultar modelos anteriores ao iPhone 12 pela CELULARS. Gostaria de confirmar disponibilidade, condição, preço final e condições de compra.')+'" target="_blank" rel="noopener">Consultar pelo WhatsApp</a></section>'}
  function renderCatalogLayout(rows){let html='';if(rows.length)html+='<section class="catalog-table-section"><div class="section-heading"><span>Tabela principal</span><strong>iPhone 12 ao iPhone 17</strong></div>'+renderTable(rows)+'</section>';html+=renderLegacyConsult();html+='<p class="table-reference-note">Preços, disponibilidade e conversão em reais são apenas referência e precisam ser confirmados via WhatsApp.</p>';return html}
