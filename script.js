(function clientMain(){
  ...
  "iPhone 16 Pro Max": {
    "Black Titanium": "design-system/device-visual-system/production-models/iphone-16-pro-max/exports/black-titanium/iphone-16-pro-max-black-titanium.png",
    ...
    "Natural Titanium": "design-system/device-visual-system/production-models/iphone-16-pro-max/exports/natural-titanium/iphone-16-pro-max-natural-titanium.png"
  },
  "iPhone 15": {
    ...
    "Black": "design-system/device-visual-system/production-models/iphone-15/exports/black/iphone-15-black.png"
  },
  "iPhone 15 Plus": {
  ...
  "iPhone 15 Pro": {
    "Black Titanium": "design-system/device-visual-system/production-models/iphone-15-pro/exports/black-titanium/iphone-15-pro-black-titanium.png",
    "White Titanium": "design-system/device-visual-system/production-models/iphone-15-pro/exports/white-titanium/iphone-15-pro-white-titanium.png",
    "Blue Titanium": "design-system/device-visual-system/production-models/iphone-15-pro/exports/blue-titanium/iphone-15-pro-blue-titanium.png",
  ...
  "iPhone 15 Pro Max": {
    "Black Titanium": "design-system/device-visual-system/production-models/iphone-15-pro-max/exports/black-titanium/iphone-15-pro-max-black-titanium.png",
  ...
  Object.entries(CDVS_IMAGE_OVERRIDES).forEach(([modelName, colors])=>{
    ...
    if(model&&model.images){
      Object.entries(colors).forEach(([color, imageConfig])=>{
        if(model.images[color]){
          const cfg=typeof imageConfig==='string'?{image_front:imageConfig}:imageConfig;
          const front=cfg.image_front||cfg.front||model.images[color].image_front||model.images[color].front||'';
          const back=cfg.image_back||cfg.back||model.images[color].image_back||model.images[color].back||'';
          const side=cfg.image_side||cfg.side||model.images[color].image_side||model.images[color].side||'';
          model.images[color]={...model.images[color],...cfg,front,image_front:front,back,image_back:back,side,image_side:side,alt:cfg.alt||modelName+" "+color};
        }
      });
  ...
  function updateExchangeCard(rateInfo,status){const rateEl=document.querySelector('[data-bcb-rate]'),dateEl=document.querySelector('[data-bcb-date]');if(rateEl)rateEl.textContent=formatBcbRate(Number(rateInfo.rate)||FALLBACK_BCB_RATE);if(dateEl){const label=status==='fallback'?'Cota\u00e7\u00e3o indispon\u00edvel temporariamente.':'Atualizada em '+formatBcbDate(rateInfo.date||rateInfo.dataHoraCotacao)+'.';dateEl.textContent=label}}
  ...
  function phoneSvgDataUri(m,c){
    const body=colorHex(c),fam=normalize((m.family||'')+' '+m.model_name),name=normalize(m.model_name),year=Number(m.year)||0;
    const home=year<=2017&&!name.includes('iphone x')&&!name.includes('iphone xs')&&!name.includes('iphone xr'),legacy=year<=2013,dynamic=year>=2023&&!name.includes('16e')&&!name.includes('se');
    const triple=fam.includes('pro')&&year>=2019,dual=!triple&&(year>=2019||name.includes('iphone x')||name.includes('iphone xs')||name.includes('plus'));
    const rearRadius=legacy?24:34,frontRadius=home?26:36;
    const lens='<circle r="9" fill="#111827"/><circle r="4" fill="#4b5563" opacity=".72"/>';
    const camera=triple?`<g transform="translate(95 60)"><rect width="66" height="66" rx="18" fill="#f8fafc" opacity=".88"/><g transform="translate(20 20)">${lens}</g><g transform="translate(46 20)">${lens}</g><g transform="translate(33 45)">${lens}</g><circle cx="48" cy="46" r="5" fill="#f5d76e"/><circle cx="20" cy="47" r="4" fill="#94a3b8"/></g>`:dual?`<g transform="translate(104 66)"><rect width="54" height="54" rx="16" fill="#f8fafc" opacity=".88"/><g transform="translate(18 18)">${lens}</g><g transform="translate(36 36)">${lens}</g><circle cx="36" cy="17" r="4" fill="#f5d76e"/></g>`:`<g transform="translate(112 74)"><rect width="42" height="42" rx="14" fill="#f8fafc" opacity=".88"/><g transform="translate(21 21)">${lens}</g></g>`;
    const topDetail=home?`<rect x="271" y="61" width="38" height="5" rx="3" fill="#0f172a" opacity=".55"/>`:dynamic?`<rect x="265" y="52" width="58" height="17" rx="9" fill="#05070b"/>`:`<path d="M252 45h84v18c0 12-10 22-22 22h-40c-12 0-22-10-22-22Z" fill="#05070b"/>`;
    const homeBtn=home?`<circle cx="294" cy="301" r="13" fill="#f8fafc" opacity=".5"/>`:'';
    const svg=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 360"><defs><linearGradient id="case" x1="0" x2="1" y1="0" y2="1"><stop offset="0" stop-color="#fff"/><stop offset=".45" stop-color="${body}"/><stop offset="1" stop-color="#e9edf3"/></linearGradient><linearGradient id="screen" x1="0" x2="1" y1="0" y2="1"><stop offset="0" stop-color="#071225"/><stop offset=".55" stop-color="#123d79"/><stop offset="1" stop-color="#dbeafe"/></linearGradient><filter id="shadow" x="-30%" y="-25%" width="170%" height="165%"><feDropShadow dx="0" dy="18" stdDeviation="15" flood-color="#0e1628" flood-opacity=".2"/></filter></defs><rect width="420" height="360" fill="none"/><g filter="url(#shadow)"><rect x="62" y="34" width="132" height="292" rx="${rearRadius+7}" fill="#111827" opacity=".18"/><rect x="70" y="42" width="122" height="276" rx="${rearRadius}" fill="url(#case)" stroke="rgba(15,23,42,.22)" stroke-width="2"/><path d="M92 55c32-11 67-9 89 6" fill="none" stroke="#fff" stroke-opacity=".45" stroke-width="3"/>${year>=2007?camera:''}<rect x="218" y="26" width="150" height="304" rx="${frontRadius+8}" fill="#0b1220"/><rect x="228" y="36" width="130" height="284" rx="${frontRadius}" fill="url(#screen)"/><path d="M246 252c41-74 83-105 113-96v70c-31 20-65 38-113 26Z" fill="#fff" opacity=".18"/><path d="M243 80c48-33 87-32 116-7v58c-39 18-85 2-116-51Z" fill="#60a5fa" opacity=".42"/>${topDetail}${homeBtn}</g></svg>`;
    return 'data:image/svg+xml;charset=UTF-8,'+encodeURIComponent(svg)
  }
  function imageRecord(m,c){const raw=(m.images&&m.images[c])||{};return{...raw,front:raw.image_front||raw.front||'',back:raw.image_back||raw.back||'',side:raw.image_side||raw.side||''}}
  function imageSourcesFor(m,c){const rec=imageRecord(m,c);return[['front',rec.front],['back',rec.back],['side',rec.side]].filter(([,src])=>Boolean(src)).map(([view,src])=>({view,src}))}
  function realImageUrlFor(m,c){const first=imageSourcesFor(m,c)[0];return first?first.src:''}
  function lightboxImagesFor(m,c){const imgs=imageSourcesFor(m,c).map(img=>img.src);return imgs.length?imgs:[phoneSvgDataUri(m,c)]}
  function imageAltFor(m,c){return imageRecord(m,c).alt||m.model_name+' '+c}
  function mediaVisualMarkup(m,c){const imgs=imageSourcesFor(m,c),fallback=phoneSvgDataUri(m,c),alt=imageAltFor(m,c);return imgs.length?'<span class="device-visual '+(imgs.length>1?'has-pair':'is-single')+'" aria-label="'+escapeHtml(alt)+'">'+imgs.map(img=>'<img class="card-device-image" src="'+escapeHtml(img.src)+'" alt="'+escapeHtml(alt+' '+img.view)+'" data-fallback-src="'+fallback+'" data-card-image data-image-view="'+escapeHtml(img.view)+'">').join('')+'</span>':'<span class="fallback-device" aria-label="'+escapeHtml(alt)+'" style="background-image:url(&quot;'+fallback+'&quot;)" data-fallback-device></span>'}
  function applyCardImage(a,m,s){const media=a.querySelector('[data-lightbox-trigger]');media.innerHTML=mediaVisualMarkup(m,s.color);media.querySelectorAll('[data-card-image]').forEach(img=>img.addEventListener('error',handleImageError))}
