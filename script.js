(function clientMain(){
  ...
  function tableModelsFor(models){return models.filter(matchesModelFilters).filter(matchesModelSearch)}
  ...
  function renderConditionPriceLine(m,capacity,condition){const p=capacityConditionPrice(m,capacity,condition);return'<div class="price-line"><span>'+escapeHtml(capacity)+'</span>'+(p?'<strong>'+formatUsd(p)+'</strong><em>'+formatBrl(p*CATALOG_RATE)+'</em>':'<strong>Sob consulta</strong>')+'</div>'}
  ...
  function renderPriceCells(m){return{ecpo:renderPriceBlock(m,'eCPO eco Certified Pre-Owned'),novo:renderPriceBlock(m,'Novo')}}
  ...
  function updateCard(a,m){const action=a.querySelector('[data-action]'),summary=a.querySelector('[data-price-summary]');if(summary)summary.innerHTML=renderBasePriceSummary(m);if(action)action.href=buildWhatsAppUrl(genericMessageFor(m))}
  ...
  function renderModelCell(m){return'<div class="model-cell"><strong>'+escapeHtml(m.model_name)+'</strong><span data-price-summary>'+renderBasePriceSummary(m)+'</span><small>Capacidades: '+escapeHtml(m.capacities.join(', '))+'</small></div>'}
  function genericMessageFor(m){return'Olá, tenho interesse no '+m.model_name+' pela CELULARS. Gostaria de confirmar capacidades, cores disponíveis, condição eCPO ou Novo, preço final e condições de compra.'}
  function defaultStateFor(m){return{capacity:m.capacities[0]||'',color:m.colors[0]||'',condition:activeCondition==='todos'?'Novo':activeCondition}}
  function renderActions(m){return'<div class="table-actions"><button class="tech-button" type="button" data-tech-trigger>Ver ficha técnica</button><a class="whatsapp-button" href="'+buildWhatsAppUrl(genericMessageFor(m))+'" target="_blank" rel="noopener" data-action>Consultar pelo WhatsApp</a></div>'}
  function renderCatalogRow(m){const s=defaultStateFor(m),cells=renderPriceCells(m),i=IPHONE_MODELS.indexOf(m);return'<tr '+rowDataAttrs(m,i,s)+'><td data-label="Modelo">'+renderModelCell(m)+'</td><td data-label="Ano"><span class="year-badge">'+m.year+'</span></td><td data-label="Cores disponíveis">'+colorList(m.colors)+'</td><td data-label="eCPO" data-price-cell="ecpo">'+cells.ecpo+'</td><td data-label="Novo" data-price-cell="novo">'+cells.novo+'</td><td data-label="Status"><span class="status-pill '+statusClass(m)+'">Status: '+escapeHtml(m.stock_status||'Consultar disponibilidade')+'</span></td><td data-label="Ação">'+renderActions(m)+'</td></tr>'}
  function renderTable(models){return'<div class="catalog-table-wrap"><table class="catalog-table"><thead><tr><th>Modelo</th><th>Ano</th><th>Cores disponíveis</th><th>eCPO</th><th>Novo</th><th>Status</th><th>Ação</th></tr></thead><tbody>'+models.map(renderCatalogRow).join('')+'</tbody></table></div>'}
